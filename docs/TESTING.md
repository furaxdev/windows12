...
| **Rollback** (annulation branding + thème + privacy-performance + first-logon + SYSTEM WSearch/SysMain) | `TESTED` — `builder/tools/test_rollback_offline.sh` : 40/40 vérifications passées (22/09/2026, après ajout de la ruche SYSTEM au module 47) contre de vraies ruches SOFTWARE/NTUSER.DAT/SYSTEM extraites de l'ISO Windows 11 25H2 officielle. Applique le forward (mêmes appels que `42-branding.sh`/`46-theme.sh`/`47-privacy-performance.sh`/`41-first-logon.sh`), vérifie par `hivexget`, applique le rollback (`hivex_delete_value.py`/`hivex_delete_key.py` pour les clés absentes en stock, ou restauration explicite à `Start=2` pour `WSearch`/`SysMain` qui existent déjà en stock), revérifie l'état attendu. Deux vrais problèmes détectés en cours de route (pas des faux positifs) : backlog #70 Spotlight, déjà natif par défaut (voir `docs/FEATURES.md`) — retiré avant merge ; et le piège du `CurrentControlSet` absent hors ligne sur la ruche SYSTEM — corrigé en lisant `\Select\Default` dynamiquement (voir `47-privacy-performance.sh`). Couvre le registre uniquement — le script PowerShell `scripts/rollback/Rollback-FuraxWindows12.ps1` destiné à l'utilisateur final n'a lui-même pas pu être EXÉCUTÉ dans un vrai Windows démarré (pas de boot possible dans cet environnement), voir ses notes internes. **Mise à jour du 25/09/2026** : PowerShell Core 7.4.6 (binaire Linux officiel, open-source) a pu être téléchargé et installé dans cet environnement, ce qui permet désormais de PARSER (pas d'exécuter contre un vrai registre) tous les `.ps1` du projet avec `[System.Management.Automation.Language.Parser]::ParseFile()` — un vrai analyseur syntaxique, pas juste une relecture humaine. Les 4 scripts `.ps1` du dépôt (`Rollback-FuraxWindows12.ps1`, `Reapply-FuraxWindows12.ps1`, `FirstLogon-Furax.ps1`, `launch-viewer.ps1`) parsent sans erreur. Ça ne remplace pas un test d'exécution réelle (pas de fournisseur registre `HKLM:`/`HKCU:` sous Linux), mais ça élimine une classe entière de bugs (le genre d'erreur de guillemets déjà rencontrée dans le `.bat` du module 49) avant même un premier test réel. |

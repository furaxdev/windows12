...
| **Rollback** (annulation branding + thème + privacy-performance + first-logon) | `TESTED` — `builder/tools/test_rollback_offline.sh` : 31/31 vérifications passées (22/09/2026, après ajout des modules 41/47) contre de vraies ruches SOFTWARE/NTUSER.DAT extraites de l'ISO Windows 11 25H2 officielle. Applique le forward (mêmes appels que `42-branding.sh`/`46-theme.sh`/`47-privacy-performance.sh`/`41-first-logon.sh`), vérifie par `hivexget`, applique le rollback (`hivex_delete_value.py`/`hivex_delete_key.py`), revérifie que tout redevient absent. Un test a détecté un vrai problème en cours de route (backlog #70 Spotlight, déjà natif par défaut — voir `docs/FEATURES.md`), retiré avant merge plutôt que forcé à passer. Couvre le registre uniquement — le script PowerShell `scripts/rollback/Rollback-FuraxWindows12.ps1` destiné à l'utilisateur final n'a lui-même pas pu être exécuté dans un vrai Windows démarré (pas de boot possible dans cet environnement), voir ses notes internes. |

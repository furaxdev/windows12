...
| Barre flottante en pilule avec marges (coins très arrondis, détachée des bords) | `PARTIAL` (compilation+dépôt `TESTED`, rendu visuel `UNCONFIRMED`) | Moyenne/Difficile | **Pivot du 26/09/2026** (demande explicite de l'utilisateur : "pas de Windhawk ou truc tiers, on dev notre propre barre") — abandon du plan Windhawk, remplacé par `apps/furax-taskbar/` : vraie application C#/.NET 8 WPF, code 100% du projet, masque la vraie barre Windows (`Shell_TrayWnd` + `ShowWindow`, API Win32 publique) et affiche 3 pilules flottantes maison par-dessus, restaurée automatiquement à la fermeture (y compris crash/fin de tâche forcée). Compilée à la volée au build (`dotnet publish -r win-x64 --self-contained`) par `builder/modules/48-taskbar-experimental.sh`, jamais committée en binaire. Flag de profil `taskbar_floating_pill_experimental` toujours désactivé par défaut. **Vérifié réellement** : la compilation produit un vrai exécutable Windows PE32+ (confirmé via `file`), et son dépôt + la clé de démarrage auto (`Run`) dans l'ISO générée sont confirmés par extraction directe du WIM. **Zone système complète (icônes tierces, flyouts) volontairement PAS réimplémentée** — sous-système reconnu comme l'un des plus complexes de tout remplacement de shell Windows, hors périmètre du MVP (icônes statiques + Win+A vers les vrais Paramètres rapides à la place). Rendu visuel réel `UNCONFIRMED` (pas de boot Windows possible dans l'environnement de dev). Voir `ui/taskbar/EXPERIMENTAL-floating-pill/README.md` pour le détail complet, y compris l'historique de la décision. |
| Widget météo intégré dans la barre | `PARTIAL` | Moyenne | `apps/furax-taskbar/Pills/WeatherPill.cs` — pilule dédiée, mêmes valeurs que la vraie frame vidéo ("28° Sunny, H:34°/L:23°"), mais **données STATIQUES** pour ce MVP (pas d'appel API météo réel — demanderait de gérer une clé API, hors périmètre de ce premier jet). Compilation + dépôt vérifiés, rendu visuel `UNCONFIRMED` — même statut que la barre flottante elle-même (#6, voir plus haut). |
...
| Fonctionnalité | État | Faisabilité | Implémentation envisagée |
|---|---|---|---|
| Fond d'écran de l'assistant Setup (écran bleu au boot USB) | `IMPLEMENTED` | Facile (une fois localisé) | `builder/modules/44-installer-background.sh` : monte `boot.wim` (index 2 "Microsoft Windows Setup", séparé d'`install.wim`), remplace `sources/background.bmp` ET `Windows/System32/setup.bmp` — malgré l'extension `.bmp` ce sont en réalité des PNG RGBA (confirmé via `file`), 1024×768 sur l'ISO 25H2 testée. Build réel exécuté avec succès (06/09/2026), fichiers confirmés remplacés après commit/démontage. **Mise à jour du 26/09/2026** (demande utilisateur — habillage visuel du Setup) : overlay texte "Windows 12" (DejaVu Sans Bold, cuit dans le PNG) + recadrage de la source (`--crop-right-frac 0.32`) pour exclure un élément graphique déjà intégré par l'artiste dans `furax-wave-primary.jpg` qui se chevauchait visuellement. **Vérifié en extrayant directement le fichier réel de `boot.wim` généré et en l'inspectant visuellement (pas une supposition)** — voir aussi la ligne "Logo/watermark" ci-dessous pour pourquoi le logo lui-même n'est PAS superposé. **Rendu au boot réel Windows Setup non vérifiable dans cet environnement** (`TESTED` réservé à ça), mais le contenu exact du fichier qui serait affiché est, lui, confirmé. |
...
| Logo/watermark additionnel sur le fond Setup (au-dessus du dégradé) | `PARTIAL` (texte fait, logo volontairement pas fait) | Facile | `prepare_installer_background.py` supporte `--logo`, mais **testé et rejeté** (26/09/2026) : `furax-wave-primary.jpg` a déjà un élément graphique en forme de grille intégré par l'artiste original (voir `assets/wallpapers/CREDITS.md`), qui se chevauche visuellement avec `windows12-logo-fanmade.png` une fois recadré à 1024×768 — constaté en inspectant l'image générée (Read tool), pas supposé. Le texte seul ("Windows 12") est utilisé à la place, la source est aussi recadrée (`--crop-right-frac`) pour exclure cet élément intégré. |
...
| 7 | Désactivation de la recherche indexée par défaut (moins de CPU/disque en fond) | `IMPLEMENTED` | `builder/modules/47-privacy-performance.sh` — ruche SYSTEM, `<ControlSetActif>\Services\WSearch\Start` = 4 (Désactivé, était 2/Automatique en stock). ControlSet actif détecté dynamiquement via `\Select\Default` (jamais codé en dur — une ruche SYSTEM démontée n'a pas de `CurrentControlSet`). Vérifié par écriture + relecture `hivexget` sur une vraie ISO générée (22/09/2026) : `Start=4` confirmé. Rollback testé offline (restaure `Start=2`, ne supprime pas la valeur — elle existe en stock) : `test_rollback_offline.sh`. |
| 8 | Superfetch/Prefetch désactivé pour SSD | `IMPLEMENTED` | `builder/modules/47-privacy-performance.sh` — même mécanisme que #7, `<ControlSetActif>\Services\SysMain\Start` = 4 (Désactivé, était 2/Automatique en stock). Vérifié par écriture + relecture `hivexget` sur une vraie ISO générée (22/09/2026) : `Start=4` confirmé. Rollback testé offline (restaure `Start=2`). |
...
| 14 | Désactivation Cortana/Copilot au premier démarrage | `PARTIAL` | Copilot déjà couvert par #61 (`TurnOffWindowsCopilot`). Cortana (l'app dédiée) a été retirée nativement de Windows 11 25H2 — il n'y a plus rien à désactiver côté Cortana sur cette version. |
| 15 | Windows Defender pré-configuré en mode "silencieux" (moins de popups) | `IMPLEMENTED` | `builder/modules/47-privacy-performance.sh` : `HKLM\Policies\Microsoft\Windows Defender Security Center\Notifications\DisableNotifications`=1 (ADMX `WindowsDefenderSecurityCenter.admx`, policy standard). Ne touche que les notifications visuelles, pas la protection elle-même. Vérifié par écriture + relecture `hivexget` sur une vraie ISO générée (22/09/2026). Rollback testé offline (40/40 PASS, `test_rollback_offline.sh`). |
...
| # | Idée | État | Note |
|---|---|---|---|
| 61 | Copilot désactivé par défaut (opt-in explicite) | `IMPLEMENTED` | `builder/modules/47-privacy-performance.sh` : `HKLM\Policies\Microsoft\Windows\WindowsCopilot\TurnOffWindowsCopilot`=1. Écrit + relu avec succès. **Complément (26/09/2026, demande explicite de l'utilisateur "juste réactiver/exposer Copilot proprement")** : `apps/furax-taskbar/Pills/DockPill.cs` ajoute un bouton Copilot dans le dock de la barre flottante maison, qui lance le VRAI Copilot Windows natif via son URI officiel (`ms-copilot:`) — pas une réimplémentation. Reste soumis à la policy `TurnOffWindowsCopilot` ci-dessus : sur un build avec `privacy_performance` actif (profil par défaut), ce bouton ne lance rien tant que l'utilisateur n'a pas retiré la policy lui-même — volontairement pas de contournement automatique depuis l'app. Compilation vérifiée (voir #6 pour le détail de la méthode de vérification de `apps/furax-taskbar`), rendu visuel réel `UNCONFIRMED` même limite que le reste de la barre. |
...
| 77 | Écran de bienvenue post-install "Bienvenue sur Furax Windows 12" | `PARTIAL` | `builder/modules/41-first-logon.sh` : dépose `FirstLogon-Furax.ps1` + écrit `HKLM\...\RunOnce\FuraxWindows12FirstLogon`. Écriture confirmée (relue dans l'ISO générée, testée offline forward+rollback 31/31 PASS). **Affichage réel du message au premier login `UNCONFIRMED`** (pas de boot Windows possible dans cet environnement). |
...
| 79 | Point de restauration auto créé juste après l'installation | `PARTIAL` | Même mécanisme que #77 : `FirstLogon-Furax.ps1` appelle `Checkpoint-Computer` au premier login (RunOnce). Écriture/dépôt confirmés. **Création réelle du point de restauration `UNCONFIRMED`** — dépend aussi de la Restauration système activée/non bloquée par policy sur la machine cible, non testable ici. |
...
| 84 | Nettoyage auto des fichiers temporaires programmé | `PARTIAL` | `scripts/first-logon/FirstLogon-Furax.ps1` : enregistre une tâche planifiée `FuraxWindows12-NettoyageTemp` via `Register-ScheduledTask` au premier login (`Remove-Item` sur le TEMP utilisateur + `Windows\Temp`, exécutée en tant que SYSTEM). Le backlog envisageait une fréquence mensuelle ; **hebdomadaire** a été choisi à la place (`New-ScheduledTaskTrigger` n'expose pas `-Monthly` sans passer par l'API CIM bas niveau, plus fragile à écrire sans pouvoir tester sur un vrai Windows démarré) — écart assumé et documenté dans le script. Dépôt du script + présence du bloc confirmés par extraction directe du WIM généré (22/09/2026). **Exécution réelle (la tâche se crée-t-elle vraiment au premier login ?) `UNCONFIRMED`**, même limite que #77/#79 (pas de boot Windows possible dans cet environnement de dev). |
...
| 96 | Détection auto de RAM dispo dans `launch-viewer.sh`/`.ps1` (ajuste `--ram` automatiquement) | `IMPLEMENTED` | **Déjà fait dans les deux scripts, doc obsolète corrigée (23/09/2026)** : `vm/launch-viewer.sh` lit `MemAvailable` dans `/proc/meminfo`, `vm/launch-viewer.ps1` lit `Win32_OperatingSystem.FreePhysicalMemory` via CIM — les deux allouent ~50% de la RAM DISPONIBLE (pas totale) à la VM, bornée entre 1024 et 4096 Mo, avec repli sur 2048 Mo si la détection échoue. `--ram`/`-Ram` reste disponible pour forcer une valeur. C'est exactement le garde-fou qui a été ajouté après le premier crash VM par RAM hôte insuffisante (voir historique du projet). |
...
| 98 | Tests automatisés du builder en CI (GitHub Actions) sur une ISO factice | `PARTIAL` | `.github/workflows/ci.yml` (23/09/2026) : valide la SYNTAXE de tout le projet à chaque push/PR (`bash -n` sur tous les `.sh`, `python3 -m py_compile` sur les outils hivex, `xmllint` sur `autounattend.xml`, parse PowerShell via l'API `[System.Management.Automation.Language.Parser]` — sans exécution). Ce n'est PAS le test "ISO factice + pipeline complet" envisagé à l'origine : fabriquer une fausse ISO exploitable par hivex nécessiterait des ruches de registre valides, et il n'existe pas de moyen simple d'en générer une "vide" sans repartir d'une vraie ruche Windows — jugé hors périmètre pour un dépôt/CI public. Vérifié localement avant push, **et confirmé pour de vrai sur GitHub Actions** (23/09/2026) : les deux premiers runs déclenchés par le push (branches `claude/furax-windows-12-beta-721he2` et `x86`) sont passés en `success` (runs #35813551471 et #35813554349, consultés via l'API GitHub Actions) — ce module est le premier de tout le projet dont l'exécution réelle a pu être vérifiée en dehors de cet environnement de développement, plutôt que seulement par relecture de fichiers. |
...
| 101 | Réapplication auto des réglages Furax après une mise à jour Windows (résiste au reset de policies/registre par Windows Update) | `PARTIAL` | Demande explicite de l'utilisateur (FuraxDev, 25/09/2026), suite à la découverte que la fidélité pixel-perfect à la vidéo (barre flottante Windhawk) implique un outil tiers fragile aux mises à jour. `scripts/reapply/Reapply-FuraxWindows12.ps1` : miroir live (cmdlets PowerShell, pas hivex) des mêmes clés que les modules 42/45/46/47. `scripts/first-logon/FirstLogon-Furax.ps1` enregistre une tâche planifiée persistante `FuraxWindows12-Reapply` (déclencheurs AtLogOn + quotidien 4h) qui le rejoue. `scripts/rollback/Rollback-FuraxWindows12.ps1` désinscrit cette tâche en premier lieu (sinon elle annulerait le rollback tout seul). Vérifié : les deux scripts déposés dans l'ISO générée sont identiques octet pour octet aux sources locales (extraction directe du WIM, 25/09/2026) ; les 4 fichiers `.ps1` du projet parsent sans erreur avec un vrai interpréteur PowerShell 7.4.6 (téléchargé et installé dans cette session — première fois qu'une vérification de syntaxe PowerShell réelle, pas juste une relecture, a été possible ici). **Ce que ça ne couvre PAS** : l'exécution réelle de la tâche planifiée sur un Windows démarré (`UNCONFIRMED`, même limite que #77/#79/#84), et la logique registre elle-même n'a que la même garantie que les modules 42/45/46/47 dont elle est le miroir — pas de nouvelle vérification au-delà. |
| 102 | Écran d'accueil "Windows 12" maison AVANT le vrai Windows Setup | `PARTIAL` | Demande explicite de l'utilisateur (FuraxDev, 26/09/2026) : "l'UI, pas l'exe système" — après avoir clairement refusé de patcher `SetupPlatform.exe` (moteur d'installation Microsoft signé : partitionnement/formatage/application d'image, un bug y détruirait des données, aucun moyen de le tester ici). `apps/furax-setup-wizard/` : app C#/.NET 8 WPF (même approche que la barre des tâches, sans XAML), fenêtre de bienvenue affichée avant `setup.exe`, qui ne partitionne/formate/installe RIEN — juste un bouton "Continuer". Accroche via `Windows\System32\winpeshl.ini` dans `boot.wim` (mécanisme WinPE officiellement documenté par Microsoft, pas un patch de binaire signé — confirmé par inspection directe que `HKLM\SYSTEM\Setup\CmdLine`="winpeshl.exe" sur l'ISO officielle et qu'aucun `winpeshl.ini` n'existe sur le média stock, recherche exhaustive). **Filet de sécurité** : `[LaunchApps]` liste `wpeinit.exe` puis notre app puis `%SYSTEMDRIVE%\sources\setup.exe` en SÉQUENCE — même si notre app plante ou ne s'affiche pas (WinPE manque potentiellement de composants de composition dont WPF dépend, risque assumé), `setup.exe` démarre quand même juste après. Vérifié réellement : build complet avec le flag activé, exe (154 Mio) ET `winpeshl.ini` extraits DIRECTEMENT du `boot.wim` généré, confirmés identiques/corrects (CRLF correct, contenu exact). **Rendu visuel réel au boot et fonctionnement de l'enchaînement `UNCONFIRMED`** — pas de boot Windows possible dans cet environnement de dev, risque explicitement assumé par l'utilisateur. Flag `setup_wizard_experimental`, désactivé par défaut partout. |

...
# login (backlog #77 message de bienvenue + #79 point de restauration auto), via le
# mécanisme RunOnce natif de Windows.
#
...
# (le point de restauration se crée-t-il vraiment ? le message s'affiche-t-il ?) n'a
# PAS pu être vérifiée dans cet environnement de développement (pas de boot Windows
# possible ici) — voir docs/FEATURES.md #77/#79, statut `PARTIAL` tant que non confirmé.
...
module_41_first_logon() {
  ...
  if [[ "${HIVEX_AVAILABLE:-0}" -ne 1 ]]; then
    log_warn "hivex indisponible — script déposé mais entrée RunOnce SKIPPED."
    mkdir -p "$script_dest_dir"
    cp -r "$script_src_dir"/* "$script_dest_dir"/
    report_step "41-first-logon" "PARTIAL" "script déposé, RunOnce SKIPPED (hivex indisponible)"
    return 0
  fi
  ...
  if /usr/bin/python3.12 "$BUILDER_DIR/tools/hivex_set_value.py" "$software_hive" \
    ...
    report_step "41-first-logon" "OK" "script déposé + RunOnce écrit (exécution réelle au boot NON vérifiée)"
  else
    log_warn "Script déposé mais échec de l'écriture de l'entrée RunOnce (voir $LOG_FILE)."
    report_step "41-first-logon" "PARTIAL" "script déposé, RunOnce FAILED"
  fi
  return 0
}
